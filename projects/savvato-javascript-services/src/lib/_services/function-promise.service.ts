...
export class FunctionPromiseService { 
	...
	*/
	initFunc(funcKey: string, func: Function) {
	...
	setFreshnessFactorInMillis(m: number) {
		this.freshnessLengthInMillis = m;
	}

	getFreshnessLengthInMillis(data: any) {
	...
	**/
	get(resultKey: string, funcKey: string, data: any) {
	...
	promiseCache: any = { }
	waitAndGet(resultKey: string, funcKey: string, data: any) {
		let self = this;
